...
DB_CONTAINER="commandframework-db"
DB_PORT=5432
DB_NAME="commandframework_test"
DB_USER="postgres"
DB_PASS="postgres"

TEST_PROJECT="tests/CommandFramework.Tests"
TEST_DLL="tests/CommandFramework.Tests/bin/Debug/net10.0/CommandFramework.Tests.dll"
MIGRATIONS_DIR="src/CommandFramework.Postgres/Migrations"

needs_rebuild() {
  [ ! -f "$TEST_DLL" ] && return 0
  [ -n "$(find src tests \( -name "*.cs" -o -name "*.csproj" \) -newer "$TEST_DLL" -print -quit)" ]
}
